...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderTree() {
        ...
        units.forEach((u, i) => {
            ...
            weeksOfUnit.forEach(w => {
                const fileHTML = w.pdf_url ? `
                 <div class="ml-12 mt-4 grid grid-cols-1 gap-4">
                     <div class="bg-surface-container-lowest p-3 rounded-lg border border-outline-variant/10 flex items-center justify-between group hover:border-primary/30 transition-all">
                        <div class="flex items-center gap-3 overflow-hidden">
                            <span class="material-symbols-outlined text-error" data-icon="picture_as_pdf">picture_as_pdf</span>
                            <div class="truncate">
                                <a href="${w.pdf_url}" target="_blank" class="text-xs font-bold text-primary hover:underline block truncate">📄 Abrir archivo</a>
                                <p class="text-[9px] text-on-surface-variant">Documento Adjunto</p>
                            </div>
                        </div>
                     </div>
                 </div>` : "";
                    ...
                    <div class="flex items-center gap-4 relative">
                        ...
                        <div class="bg-white/80 backdrop-blur p-3 rounded-lg shadow-sm border border-outline-variant/10 flex items-center gap-3 w-fit min-w-[240px]">
                            ...
                            <div>
                                ...
                                <p class="text-[9px] font-bold text-secondary uppercase">${w.descripcion || 'Sin descripción'}</p>
                            </div>
                        </div>
                    </div>
                    ${fileHTML}
    ...
    btnUploadWork.addEventListener("click", async () => {
        ...
        const { data, error } = await supabaseClient.from('semanas').update(payload).eq('id', semana_id);

        if (error) {
            alert("Error al actualizar la base de datos: " + error.message);
        } else {
            alert("Trabajo subido y enlazado exitosamente.");
